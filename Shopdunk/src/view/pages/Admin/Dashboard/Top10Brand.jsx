...
import { getTop10SubProd, getTop10Prod, getTop10Users, getTop10SubCate, getTop10Brand } from "../../../../services/dashboardService";
...
export default function TopBrand() {
    ...
    const handleGetListData = async (year,month)=>{
        let rs = await getTop10Brand(year,month);
        if(rs && rs?.length>0){
            console.log(rs);
    ...
    useEffect(() => {
      ...
      const labels = data.map(
        (item) =>  item.brand ? `${item.brand}` : "Khác"
      );
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4">
      ...
      {chartData.datasets[0].data.length > 0 ? (
        ...
        style={{
          ...
          📊 Top 10 thương hiệu có lượt mua nhiều nhất theo {selectedTime.month !== null ? `tháng ${selectedTime.month}`: ``} năm {selectedTime.year}
        </h2>
